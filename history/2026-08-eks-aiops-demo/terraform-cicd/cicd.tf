...
resource "helm_release" "argocd" {
  ...
  values = [
    templatefile("${path.module}/helm-values/argocd.yaml", {
      hostname              = "argocd.${var.hosted_zone_name}"
      server_admin_password = htpasswd_password.argocd.bcrypt
      gateway_name          = var.gateway_name
      gateway_namespace     = var.gateway_namespace
      gateway_listener      = var.gateway_listener

      # 배포 실패 시 kagent 에이전트가 조사를 시작하도록 알림을 보낼 곳.
      # Alertmanager와 같은 릴레이를 공유함 (이유는 kagent-alert-relay.tf 주석 참고).
      kagent_relay_url = var.kagent_relay_url
    })
  ]
